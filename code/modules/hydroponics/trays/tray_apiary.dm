	...
	if(world.time > (lastcycle + cycledelay))
		...
		if(nutrilevel > 0)
			...
			if(owned_bee_swarms.len)
				var/mob/living/simple_animal/hostile/retaliate/bee/B = pick(owned_bee_swarms)
		...
		if(bees_in_hive >= 10 && prob(bees_in_hive * 10))
			var/mob/living/simple_animal/hostile/retaliate/bee/B = new(get_turf(src), src)
...
/obj/machinery/apiary/proc/angry_swarm(var/mob/M)
	for(var/mob/living/simple_animal/hostile/retaliate/bee/B in owned_bee_swarms)
		B.feral = 25
		B.target_mob = M
